...
## Features
- Three views: Cards (tiles with quick edit/delete/duplicate), List (dense rows with Title, Date, Time/All-day, Location, Show As, Reminder, Priority), and Calendar (month/week/day grid with click-to-add and click-to-edit).
- Quick Add row for rapid multi-entry: Title + Date + Time + Preset in one line.
- Presets for duration/reminder/show-as to speed repetitive meeting entries (saved in localStorage).
- Bulk edit panel for selected events (date/time/reminder/show-as/all-day).
- CSV import via toggleable panel (upload or paste).
...
- Duplicate: Use Duplicate on a card/list row to create a copy and open it for editing.
...
Quick Add
- Enter Title, Date, and Time, then click Quick Add to insert an event without opening the modal.
- Choose a Preset to auto-apply duration, reminder, and show-as.

Presets
- Create a preset by setting Duration, Reminder, and Show As, then click Save Preset.
- Presets are saved to localStorage so they persist between sessions.

Bulk Edit
- Select one or more events, then use the Bulk Edit panel to set date/time/reminder/show-as/all-day across the selection.

CSV Import
- Click Import CSV in the top actions to reveal the import panel.
- Upload a CSV or paste CSV text, then click Import.
- Supported headers: `title,startDate,startTime,endDate,endTime,location,description,allDay,reminder,showAs,priority`.
...
- Timezone: Add a timezone selector and optional UTC (Z) exports.
